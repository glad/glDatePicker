...
B,g.render()))});c.append(e).append(k).append(l);for(l=e=0;7>e;e++)for(var q=0;7>q;q++,l++){var C=new Date(f),n="day",r=b.zIndex+l,v=$("<div/>");if(e){C._add(q+7*(e-1));var p=C._val(),w=p.time,K=null,x=!0,D=function(b,a){!0===b.repeatYear&&a.setYear(p.year);!0===b.repeatMonth&&a.setMonth(p.month);return a._val()};v.html(p.date);b.selectableDateRange&&(x=!1,$.each(b.selectableDateRange,function(b,a){var c=a.from,d=a.to||null,d=d||new Date(a.from.getFullYear(),a.from.getMonth(),a.from._max()),c=D(a,
c),d=D(a,d);if(w>=c.time&&w<=d.time)return x=!0}));b.selectableDates&&(b.selectableDateRange&&!x&&(x=!1),$.each(b.selectableDates,function(b,a){if(D(a,a.date).time==w)return x=!0}));!x||0>u.indexOf(p.year)||0>H.indexOf(p.month)||0>y.indexOf(p.day)?n="noday":(n="sun mon tue wed thu fri sat".split(" ")[p.day],I!=p.month&&(n+=" outday"),t==w&&(n="today",r+=50),b.selectedDate._time()==w&&(n="selected",r+=51),b.specialDates&&$.each(b.specialDates,function(b,a){D(a,a.date).time==w&&(n=a.cssClass||"special",
r+=52,K=a.data)}),v.mousedown(function(){return!1}).hover(function(a){a.stopPropagation();a=$(this).data("data");b.onHover(d,v,a.date,a.data)}).click(function(a){a.stopPropagation();a=$(this).data("data");b.selectedDate=b.firstDate=a.date;g.render(function(){!b.showAlways&&b.hideOnClick&&g.hide()});b.onClick(d,$(this),a.date,a.data)}))}else n="dow",v.html(z[q]),C=null;$.extend(j,{borderTopWidth:h,borderBottomWidth:h,borderLeftWidth:0<e||!e&&!q?h:0,borderRightWidth:0<e||!e&&6==q?h:0,marginLeft:0<q?
"-"+h:0,marginTop:0<e?"-"+h:0,zIndex:r});v.data("data",{date:C,data:K}).addClass(" core border "+n).css(j);c.append(v)}var N=function(a){b.allowMonthSelect&&(L.css({display:!a?"none":"inline-block"}),E.css({display:!a?"inline-block":"none"}));b.allowYearSelect&&(M.css({display:a?"none":"inline-block"}),F.css({display:a?"inline-block":"none"}))},t=function(){b.firstDate=new Date(F.val(),E.val(),1);g.render()},E=$("<select/>").hide().change(t),F=$("<select/>").hide().change(t),L=$("<span/>").html(s[I]).mousedown(function(){return!1}).click(function(a){a.stopPropagation();
